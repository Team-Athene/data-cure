...
<template>
  <div v-if="type === 'header'">
    ...
    <ABtn
      color="info"
      class="rounded-full px-6 py-5 font-bold mr-4 w-36 text-base">
    <AMenu trigger="hover">
      <AList 
    ...
    <ABtn v-if="isAuthenticated"
      class="rounded-full px-6 py-5 font-bold text-base" color="primary" @click="disconnectWallet()">
      Logout
      <ALoadingIcon icon="i-bx-log-out" />
    </ABtn>
    <ABtn
      v-else
      class="rounded-full px-6 py-5 font-bold text-base"
      color="primary"
      @click="connectWallet()"
    >
      Login
