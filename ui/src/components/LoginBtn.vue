...
const { userInfo, walletClient, contracts, publicClient } =
...
const retrieveMessages = async () => {
  ...
  console.log(
    '🚀 ~ file: access-requests.vue:28 ~ retrieveMessages ~ ContentTopic:',
    ContentTopic,
  )
  ...
  const callback = (wakuMessage: any) => {
    ...
    console.log(
      '🚀 ~ 🚀 ~ 🚀 ~ file: WakuVerification.ts:62 ~ useWakuVerification ~ callback ~ messageValue:',
      messageValue,
    )
  ...
  if (safeAuthPack.value!.isAuthenticated) {
    ...
    contracts.value = {
      ...
      DataCureAccess: getContract({
        address: ContractAddresses.DataCureAccess[
          userInfo.value.network
        ] as any,
        publicClient: publicClient.value,
        walletClient: walletClient.value,
...
<template>
  <div v-if="type === 'header'">
    ...
    <ABtn
      ...
      :color="selectedNetwork == 'none' ? 'danger' : 'info'"
    ...
    <ABtn
      v-if="isAuthenticated"
      class="rounded-full px-6 font-bold text-sm"
      color="primary"
      @click="disconnectWallet()"
    >
      Logout {{ userInfo.walletAddress?.slice(0, 6) }}...{{
  ...
  <div v-if="type === 'register'">
    <ABtn
      class="rounded-full w-72 mt-4 mb-6 font-bold"
      v-if="!isAuthenticated"
      color="primary"
      @click="connectWallet()"
    >
